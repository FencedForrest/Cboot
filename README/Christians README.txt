...
--Useful commands during this Process--
 ...
 ansible -m ping all - to ping the containers in Ansible this will confirm you have access 
 to them

-Issues Encountered -

- Along the way I encountered a few issues, do to the limitations of the free account I had to connect all the VMs ELK server via its public IP. In addition to this ensuring that all the firewall and load balancer rules where in order to make the communication work. Unfortunately when I created the second DVWM server I forgot to add it to the same availability group which can only be done during the creation phase. This caused me to have to delete and remake the second server. Hopefully this will help you avoid the same issues I had.
